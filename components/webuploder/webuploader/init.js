                            ...
                            if (uploadedFiles.length > 0) {
                                if (!config.pick.multiple) {
                                    ...
                                    $.each(uploadedFiles, function(idx, url) {
                                        if (url.code == 0) {
                                            chooseObject.parent().parent().next().append('<div class="multi-item"><img src="'+url.url+'" class="img-responsive img-thumbnail cus-img" data-toggle="modal" data-target="#select_image"><input type="hidden" name="'+name+'[]" value="'+url.attachment+'"><input type="hidden" name="'+id+'[]" value="0"><em class="close delMultiImage" title="删除这张图片">×</em></div>');
                                        } else {
                                            if (!multierr) {
                                                multierr = true;
                                                alert(url.msg);
